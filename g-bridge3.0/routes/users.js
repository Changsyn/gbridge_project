...
let htmlstream='';
    ...
    if (body.uid == '' || body.pw1 == '') {
       ...
       db.query('INSERT INTO u32_users (uid, pass, name, addr, phone ) VALUES (?, ?, ?, ?, ?)', [body.uid, body.pw1, body.uname, body.addr, body.phone], (error, results, fields) => {
          if (error) {
...
const HandleLogin = (req, res) => {
  ...
  let htmlstream = '';
      ...
      if (body.uid == '' || body.pass == '') {
       ...
       sql_str = "SELECT uid, pass, name from u32_users where uid ='"+ body.uid +"' and pass='" + body.pass + "';";
...
// --------------- 정보변경 기능을 개발합니다 --------------------
const PrintUinfomod = (req, res) => {
    let    htmlstream = '';

         htmlstream = fs.readFileSync(__dirname + '/../views/header.ejs','utf8');
         htmlstream = htmlstream + fs.readFileSync(__dirname + '/../views/navbar.ejs','utf8');
         htmlstream = htmlstream + fs.readFileSync(__dirname + '/../views/uinfomod_form.ejs','utf8');
         htmlstream = htmlstream + fs.readFileSync(__dirname + '/../views/footer.ejs','utf8');
         res.writeHead(200, {'Content-Type':'text/html; charset=utf8'});
  ...
  // 회원정보 변경 양식에 따라 입력된 변경사항들을 DB에 저장합니다.
  const HandleUinfoModification = (req, res) => {  //정보 변경
  let body = req.body;
  let htmlstream='';

    // 임시로 확인하기 위해 콘솔에 출력해봅니다.
      console.log(body.cpw1);
      console.log(body.caddr);
      console.log(body.cphone);

      if (body.cpw1 == '' || body.caddr == '' || body.cphone == '') {
           console.log("변경할 데이터가 입력 되지 않아 DB에 저장할 수 없습니다.");
           res.status(561).end('<meta charset="utf-8">변경할 데이터가 입력되지 않아 가입을 할 수 없습니다');
      }
      else {

         db.query('UPDATE u32_users set pass=? where uid=?',
          [body.cpw1, req.session.who], (error, results, fields) => {
            if (error) {
              htmlstream = fs.readFileSync(__dirname + '/../views/alert.ejs','utf8');
              res.status(562).end(ejs.render(htmlstream, { 'title': '알리미',
                                 'warn_title':'회원가입 오류',
...
router.get('/profile', PrintUinfomod);     // 정보변경화면을 출력
router.put('/reg', HandleUinfoModification);   // 회원가입내용을 DB에 등록처리
